<script>
    ...
    function getLevels(l_id) {
        var url = '<?php echo e(route('get_levels', [':id'])); ?>';
    ...
    function getPrograms(p_id) {
        var url = '<?php echo e(route('get_programs', [':id'])); ?>';
    ...
    function getStates(t_id) {
        var url = '<?php echo e(route('get_states', [':id'])); ?>';
    ...
    function getTowns(state_id) {
        var url = '<?php echo e(route('get_towns', [':id'])); ?>';
...
//        Update Values
    ...
    function updateExamResults(classID) {
        ...
        let url = '<?php echo e(route('classExamUpdateTotal', [':id'])); ?>';
    ...
    function updateExamResultsToPublish(classID) {
        ...
        let url = '<?php echo e(route('resultsPublish', [':id'])); ?>';
        url = url.replace(':id', classID);
        // Perform an AJAX request to update the database with the new value
        // You can use Laravel's route and controller for this
        console.log(newValues);
    ...
    function EnterResults(classID) {
        ...
        let url = '<?php echo e(route('postedResults.process')); ?>';
        //url = url.replace(':id', classID);
        // Perform an AJAX request to update the database with the new value
        // You can use Laravel's route and controller for this
        //console.log(newValues);
    ...
    $('#submitButton').on('click', function () {
        ...
        let url = '<?php echo e(route('BoardofExaminersUpdateResults')); ?>';
...
// Get the value of the last radio button that was clicked.
    ...
    $('.publish-results-board').click(function (e) {
        ...
        if (ids != '') {
            ...
            $.ajax({
                url: '<?php echo e(route('publishProgramResults')); ?>',
                type: 'POST',
                data: {
                    _token: $('meta[name="csrf-token"]').attr('content'),
                    ids: ids,
    ...
    function LoadMoreResultsCas(current_page, last_page, per_page, program, academic) {
        ...
        $.ajax({
            url: '<?php echo e(route('load.more.results.board.Cas')); ?>',
            type: 'POST',
            data: {
                _token: $('meta[name="csrf-token"]').attr('content'),
...
<th>Course Code</th>
    ...
    function LoadMoreResults(current_page, last_page, per_page, program, academic) {
        ...
        $.ajax({
            url: '<?php echo e(route('load.more.results.board')); ?>',
            type: 'POST',
            data: {
                _token: $('meta[name="csrf-token"]').attr('content'),
                current_page: current_page,
...
function downloadCSVtemplate(){
    let url = '<?php echo e(route('template.download')); ?>';
   ...
   /*     var ctxs = document.getElementById("BestBasedOnClass").getContext('2d');
        var data = <?php echo (isset($grouped) && !empty($grouped) ? $grouped:'00'); ?>;
        var studentNames = [];
        var totals = [];
...
//second chart
            ...
            var data = <?php echo (isset($results) && !empty($results)) ? json_encode($results) : '00'; ?>;
            var studentName = [];
            var total = [];
            var end = 0;
...
//second chart
            ...
            var report = <?php echo (isset($results) && !empty($results)) ? json_encode($results) : '00'; ?>;
    ...
    function getLevelAssess(program) {
        ...
        var academic = $('input[name="academic"]').val();
        $.ajax({
            url: '<?php echo e(route('get_levelsAsses')); ?>',
            type: 'POST',
            data: {
                programID: program,
                academicPeriodID: academic
...
<?php /**PATH /Users/lavumweemba/Documents/zictcollegenewsms/zictcollege/resources/views/partials/js/custom_js.blade.php ENDPATH**/ ?>