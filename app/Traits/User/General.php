...
use App\Models\Admissions\AdmissionStatus;
...
use App\Models\Applications\AdmissionStatusRequest;
use App\Models\Applications\Booking;
use App\Models\Applications\ChangeProgram;
use App\Models\Applications\ChangeStudyModeRequest;
...
use Invoicing;
    ...
    public static function jsondata($id)
    {
        ...
        $AcademicProgression = self::getAcademicPaymentProgress($user->id, $apID);
        ...
        if ($currentProgram) {
            $progression = self::checkProgression($user->id, $currentProgram['id']);
        } else {
            $progression = [];
        }
        ...
        $user = [
            ...
            'changeProgramRequests'    => ChangeProgram::usersChangeProgramRequests($user->id),
            'exemptions'               => Exemption::myExemptions($user->id),
            'admissionStatus'          => self::currentUserAdmissionStatusValue($user->id),
            'withdrawDeferments'       => WithDrawDeferment::submissions($user->id),
            'admissionStatusRequests'  => AdmissionStatusRequest::requests($user->id),
            'addDropCourseRequests'    => AcAddDropCourseRequests::userRequests($user->id),
            'reAdmissionRequests'      => StudentAdmissionRequest::requests($user->id),
            'changeStudyModeRequests'  => ChangeStudyModeRequest::requests($user->id),
            'roles'                    => 'to be updated',
    ...
    public static function userFinancialAndAcademicStatus($id,$academicPeriodID)
    {
        ...
        $user = [
            ...
            'sms_id'                   => $user->guest_id,
            'student_id'               => $student_id,
            'balance'                  => number_format((new PaymentPlan)->balance($user->id)),
            'last_login'               => $user->last_login_at,
            'last_seen'                => $last_seen,
            'ip_address'               => $user->last_login_ip,
            'avatar'                   => "/user_photos/" . '' . $user->image,
    ...
    public static function currentUserAdmissionStatusValue($userID)
    {
        ...
        if (!empty($currentProgram)) {
            $admissionStatus        = AdmissionStatus::where('userID', $userID)->where('programID', $currentProgram->programID)->get()->last();
    ...
    public static function currentUserAdmissionStatus($userID)
    {
        $admissionStatus    = [];
        $currentProgram     = UserProgram::where('userID', $userID)->get()->last();
        $admissionStatus    = AdmissionStatus::where('userID', $userID)->where('programID', $currentProgram->id)->get()->first();
    ...
    public static function jsondataBasic($id)
    {
        ...
        $semesterStatus     = self::checkStudentsSemester($user->id);
        ...
        if (is_numeric($q)) {
            ...
            if (!empty($nrcs)) {
                ...
                foreach ($nrcs as $nrc) {
                    ...
                    $user = $nrc->user;
                    $user = self::jsondata($nrc->user->id);
                    $users[] = $user;
                }
                if (!empty($users)) {
                    return $users;
    ...
    public static function userBalanceReport($userID)
    {
        ...
        return [
            ...
            'student_id'               => $student_id,
            'balance'                  => (new PaymentPlan)->balance($user->id),
            'balanceReadable'          => env('BILLING_CURRENCY') . ' ' . number_format((new PaymentPlan)->balance($user->id)),
        ];
    }
    ...
    public static function userBalanceReportWithPaymentPlan($userID)
    {
        ...
        $apID                = self::getCurrentAcademicPeriodID($userID);
    ...
    public static function getAcademicPaymentProgress($userID, $AcademicPeriodID)
    {
        ...
        $percent = self::AcademicPaymentPercentage($userID, $AcademicPeriodID);
    ...
    public static function checkStudentsSemester($userID)
    {
        ...
        $currentAcademicPeriodID = self::getCurrentAcademicPeriodID($userID);
        $checkStatus             = self::checkIfInvoiceHasMadetoryFees($userID, $currentAcademicPeriodID);
    ...
    public static function getUserPaymentPercentage($userID, $academicID)
    {
        ...
        $AcademicProgression = self::getAcademicPaymentProgress($userID, $academicID);
        $percent             = round(self::AcademicPaymentPercentage($userID, $academicID));
